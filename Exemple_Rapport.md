...
<ol style="list-style-type : none">
  ...
  <li> <a href="#3"> Exemple d'utilisation simple avec la base SnowLooker </a>
    <ol>
      ...
      <li> <a href="#3.5"> Création de visualisations dans un Dashboard </a>
        <ol>
          <li> <a href="#3.5.1"> Une valeur </a> </li>
          <li> <a href="#3.5.2"> Un graphique </a> </li>
        </ol>
      </li>
...
<p> Le modèle et le dashboard que nous allons créer n’est qu’un simple exemple, Looker permet de faire plus de choses et dont certaines qui peuvent être faites de plusieurs façons. </p>
...
<p> Les résultats que vous obtiendrez si vous réalisez cet exemple ne seront sûrement pas les mêmes que sur les captures d’écran de ce rapport puisque la base de données est mise à jour quotidiennement. </p>
<br/>
<br/>
...
<p> Avant d’établir une connexion, il faut configurer sa base de données pour Looker. <br/>
Documentation : <a href="https://docs.looker.com/setup-and-management/database-config"> https://docs.looker.com/setup-and-management/database-config </a> </p>
<p> Puis, il faut ajouter la connexion en allant dans l’onglet "Admin" -> "Connections" puis sur "Add Connection". <br/>
Documentation : <a href="https://docs.looker.com/setup-and-management/connecting-to-db"> https://docs.looker.com/setup-and-management/connecting-to-db </a> </p>
...
Cette table dérivée reliant chaque client à ses commandes permettra de connaitre des informations sur les commandes de chaque client. Par exemple, on pourra calculer le nombre de commandes du client, la date de sa première commande…  </li>
</ul>
...
explore: order_items {       <i style="color:dodgerblue"> # vue inclue dans l’Explore </i> <br/>
  &nbsp;&nbsp;label: "Commandes et utilisateurs"    <i style="color:dodgerblue"> # nom affiché dans l'onglet Explore </i> <br/>
  &nbsp;&nbsp;view_label: "Commandes"           <i style="color:dodgerblue"> # optionnel : nom de la vue qui apparaitra dans le sélecteur de champs </i> <br/>
...
<p> <b> Par la suite, il faudra mettre à jour le dépôt Git quand on fait des changements : </b> </p>
<ul>
  <li>  Cliquer sur "Commit Changes" puis éventuellement sur "Pull and merge other Changes" permet de partager les changements qu’on a effectué sur le projet avec les autres développeurs ainsi que de charger les modifications effectuées par d’autres développeurs. </li>
  <li> Cliquer sur "Deploy to Production" permet de partager le projet avec les utilisateurs non développeurs (qui ne sont pas en mode développement). Cela leur permettra, en fonction de leurs droits, d’explorer les données, de créer, modifier et visualiser des contenus (Looks et Dashboards).</li>

</ul>
<p> Une fois que le projet est partagé avec la production, on peut quitter le mode développement pour créer des visualisations (sauf si on veut continuer à développer). Si le projet n'est pas partagé alors il faut rester ne mode développement pour créer les visualisations mais elles ne seront pas visibles par les autres utilisateurs. </p>
<br/>
<br/>
...
<ul>
  ...
  <li>  Sélectionner les dimensions et les mesures que l’on souhaite représenter c’est-à-dire "Majeur" et "Count" sous le label "Utilisateurs". </li>
...
<ul>
  ...
  <li>  Aller dans "EDIT" pour modifier le nom de l’axe des ordonnées dans le volet "X". Aussi, dans le volet "Series", modifier le type de points pour "Outline" et changer la couleur de la courbe : </li>
  <p style="text-align:center"> <img src="https://raw.githubusercontent.com/EmelineCailliau/ec_exemple/master/Images/ex7.png"/> </p>
  <li>  On va sauvegarder cette visualisation sans l’ajouter à un Dashboard, cela permettra plus tard, si on le souhaite de l’insérer à plusieurs Dashboards. En effet, quand on sauvegarde seulement une visualisation dans un Dashboard, cela ne permet pas de l’insérer à d’autres Dashboards par la suite : </li>
...
<p> On va insérer la visualisation "Evolution du nombre d'items vendus par mois", que nous avions sauvegardée dans notre espace personnel, au Dashboard  que nous avions créé dans ce même espace : <p>

<ul>
  <li>  Aller dans l’onglet "Browse" → "<i>YourName</i> ’s Space". <br/>
    <i> <b style="text-decoration:underline">Remarque :</b> Dans cet espace (ou n’importe quel autre espace), on peut créer un nouveau Dashboard en allant sur "New" → "Dashboard" en haut à droite. On peut aussi créer des sous-espaces (sorte de dossier) en allant sur "New" → "Space". C’est aussi dans cet espace qu’on retrouve la visualisation "Evolution du nombre d'items par mois". </i>
  </li>
...
<p> On va ajouter un filtre sur le pays de l’utilisateur : </p>
<ul>
...
<ul>
  <li>  Ouvrir le Dashboard : onglet "Browse"  → "<i>YourName</i> ’s Space". </li>
...
<p> Looker propose peu de "map_layer" donc si notre dimension ne correspond à aucun "map_layer" prédéfinit, on peut le définir nous même (en important un fichier de données spatiales au format topojson). </p>
<br/>
<br/>
...
<p> On peut masquer une colonne qu’on a sélectionnée afin qu’elle n’apparaisse pas dans la visualisation : </p>
<p style="text-align:center"> <img src="https://raw.githubusercontent.com/EmelineCailliau/ec_exemple/master/Images/ex24.PNG"/> </p>
<p> Cela permet notamment d’utiliser des champs pour créer des colonnes calculées ("Calculations") sans que ceux-ci n'apparaissent dans la visualsiations. </p>
<br/>
<br/>
...
<p> On peut remplacer les points par des icones en modifiant le type des points dans le volet "Points" puis choisir l’icône : </p>
...
<p> Pour choisir la palette de couleurs par défaut des visualisations, aller dans "Admin" → "Settings" puis dans la partie "Default Visualization Colors" (ne pas oublier de cliquer sur "Update" pour enregistrer les modifications).
...
<p> L’url du lien permet aussi de conserver les valeurs des filtres au cours de l’exploration, par exemple si on veut conserver la valeur du filtre sur la dimension "created_date" de la table "order_items" en l’affectant au filtre "Période" (du dashboard "customer_lookup") : </p>
...
<p> Un exemple d’utilisation de l’action "SendGrid" permettant d’envoyer des mails est présent dans la vue "02_users" du projet "welcome to Looker" pour la dimension "email". <br/>
...
<p> Par exemple si on veut que les managers des centres de distributions n’aient accès qu’aux données de leur centre de distribution : </p>
<ul>
...
<ul>
  <li>  <b> Créer un ensemble de modèles : </b> aller dans l’onglet "Admin" → "Roles" puis cliquer sur "New Model Set", sélectionner le ou les modèle(s) pour lesquels on veut donner l’accès. </li>
  <li>  <b> Créer un rôle : </b> aller dans l’onglet "Admin" → "Roles" puis cliquer sur "New Role", choisir les permissions que l’on souhaite accorder ("Permission Set") et le "Model Set" que l’on vient de créer. Si l’on veut, on peut aussi sélectionner les utilisateurs ou/et groupes d’utilisateurs qui auront ce rôle.
  Attention : même si on a attribué ce rôle à un utilisateur, il est possible qu’il ait quand même accès aux autres modèles s’il possède un autre rôle le permettant, par exemple, le rôle d’administrateur (qui lui donne tous les droits). </li>
...
<ul>
  <li>  Aller dans l’onglet "Browse" → "<i>YourName</i> ’s Space". </li>
